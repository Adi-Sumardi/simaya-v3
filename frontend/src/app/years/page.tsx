...
import { Calendar, Plus, Search, Menu, Edit, Trash, Loader2 } from "lucide-react";
    ...
    <div className="flex bg-background min-h-screen relative overflow-x-hidden">
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground font-medium mt-0.5">Kelola data list tahun pengadaan dan kodenya untuk barcode aset.</p>
              </div>
            </div>
          </div>
          <Button
          ...
          <CardContent className="p-0">
            {loading ? (
              <div className="flex flex-col items-center justify-center py-20 gap-3">
                ...
                <p className="text-xs text-muted-foreground font-semibold">Memuat data tahun...</p>
              </div>
            ) : (
              <>
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead>Kode Singkat</TableHead>
                  ...
                  <TableBody>
                    {years.map((y) => (
                      <TableRow key={y.id}>
                        <TableCell className="font-extrabold text-foreground">{y.year}</TableCell>
                        <TableCell className="font-bold text-primary font-mono">{y.code}</TableCell>
      ...
      <Dialog open={createModal} onOpenChange={setCreateModal}>
        <DialogContent className="max-w-md rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-base font-extrabold font-serif">Tambah Tahun Pengadaan</DialogTitle>
            <DialogDescription className="text-xs">
              Tambahkan data tahun perolehan/pengadaan aset.
            </DialogDescription>
          </DialogHeader>
          <form onSubmit={handleCreate} className="flex flex-col gap-4 mt-2">
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="year-val">Tahun</Label>
              <Input
                id="year-val"
                type="text"
                maxLength={4}
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="year-code">Kode Singkat (Max 4 Karakter)</Label>
              <Input
                id="year-code"
                type="text"
                maxLength={4}
      ...
      <Dialog open={!!editModal} onOpenChange={(open) => !open && setEditModal(null)}>
        <DialogContent className="max-w-md rounded-3xl p-6">
          <DialogHeader>
            <DialogTitle className="text-base font-extrabold font-serif">Ubah Tahun Pengadaan</DialogTitle>
            <DialogDescription className="text-xs">
              Perbarui data tahun atau kode pengadaan aset.
            </DialogDescription>
          </DialogHeader>
          {editModal && (
            <form onSubmit={handleEdit} className="flex flex-col gap-4 mt-2">
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="edit-year-val">Tahun</Label>
                <Input
                  id="edit-year-val"
                  type="text"
                  maxLength={4}
              ...
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="edit-year-code">Kode Singkat (Max 4 Karakter)</Label>
                <Input
                  id="edit-year-code"
                  type="text"
                  maxLength={4}
